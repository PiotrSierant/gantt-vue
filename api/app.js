...
app.get('/api/dashboard-zegary', (req, res) => {
    const sql = 'SELECT id FROM process'; // Pobieramy tylko ID
    db.query(sql, (err, results) => {
        if (err) return res.status(500).json({ error: err });
        const zegaryIds = results.map((row) => row.id);
        res.json(zegaryIds);
    });
});

app.get('/api/dashboard-zegary/:id', (req, res) => {
    const { id } = req.params;
    const sql = `
        SELECT 
            process.*, 
    ...
    db.query(sql, [id], (err, results) => {
        if (err) return res.status(500).json({ error: err });
        if (!results.length) return res.status(404).json({ error: 'Zegar nie znaleziony' });
